...
Arquivo: `src/main/java/io/github/sidneyroberto9/rotom/date/RotomDateUtils.java`

Classe utilitária para conversões, checagens temporais e formatação de datas. **Não** cobre dias
úteis/feriados (veja [`RotomDateService`](../RotomDateService/README.md) para isso).
...
```java
RotomDateUtils dateUtils = new RotomDateUtils();
```

## Bean Spring

Com `RotomAutoConfiguration` ativo, um bean `RotomDateUtils` fica disponível para injeção.
