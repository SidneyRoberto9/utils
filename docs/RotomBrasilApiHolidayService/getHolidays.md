# RotomBrasilApiHolidayService.getHolidays(int year)

**Pacote:** `io.github.sidneyroberto9.rotom.date.holiday`
**Arquivo:** `RotomBrasilApiHolidayService.java`
...
- **Lança `IOException`** se a requisição HTTP falhar (rede indisponível, timeout) ou retornar
  ...
  [`RotomCepService.lookup`](../RotomCepService/lookup.md), que engole falhas de provider individuais).
