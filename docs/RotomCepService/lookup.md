# RotomCepService.lookup(String rawCep)

**Pacote:** `io.github.sidneyroberto9.rotom.cep.domain`
**Arquivo:** `RotomCepService.java`
...
- **Lança `IllegalArgumentException("CEP inválido: " + rawCep)`** se o CEP, após normalizado, não
  tiver exatamente 8 dígitos (via [`RotomCepUtils.normalize`](../RotomCepUtils/normalize.md) +
  [`isValid`](../RotomCepUtils/isValid.md)).
